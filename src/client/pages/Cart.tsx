...
interface DeliveryInfo {
  ...
  // shippingZoneId is NOT stored on cart; zone is derived on the backend
  deliveryFee?: number;
}
  ...
  return (
    <div className="fixed bottom-4 right-4 z-50 flex flex-col gap-2">
      {toasts.map((toast) => (
        <div
          key={toast.id}
          className={`flex items-center gap-3 rounded-lg px-4 py-3 shadow-lg animate-in slide-in-from-right-5 fade-in duration-300
        ...
        >
          ...
          {toast.type === "info" && <Shield className="h-5 w-5 text-blue-500" />}
          <span className="text-sm font-medium">{toast.message}</span>
          <button onClick={() => removeToast(toast.id)} className="ml-2 rounded p-0.5 hover:bg-gray-200/50">
            <X className="h-4 w-4" />
          </button>
        </div>
      ))}
    ...
    <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
      <div className="mb-8 flex flex-wrap items-center justify-between gap-4">
        <div className="space-y-2">
          <div className="relative h-9 w-48 overflow-hidden rounded-lg bg-gray-200"><Shimmer /></div>
          <div className="relative h-4 w-64 overflow-hidden rounded-lg bg-gray-200"><Shimmer /></div>
        </div>
        <div className="relative h-10 w-36 overflow-hidden rounded-lg bg-gray-200"><Shimmer /></div>
      </div>
    </div>
  </div>
);
  ...
  return (
    <div className="flex min-h-[70vh] flex-col items-center justify-center gap-6 text-center">
      ...
      <div className="space-y-2">
        ...
        <p className="text-gray-500 max-w-md">Please sign in to access your shopping cart and proceed with your order.</p>
      </div>
      <div className="flex flex-wrap gap-4 justify-center">
        ...
        >
          <LogIn size={18} className="transition-transform group-hover:translate-x-0.5" /> Sign In
        </button>
        <button
          onClick={() => navigate("/")}
        ...
        >
          Continue Shopping <ArrowRight size={18} className="transition-transform group-hover:translate-x-1" />
        </button>
      </div>
    </div>
  );
...
const EmptyCart = () => (
  <div className="flex min-h-[70vh] flex-col items-center justify-center gap-6 text-center">
    ...
    <div className="space-y-2">
      ...
      <p className="text-gray-500 max-w-sm">Looks like you haven't added any items yet.</p>
    </div>
    <Link
      to="/"
...
const CartItemRow = ({ item, onIncrease, onDecrease, onRemove, isUpdating }: CartItemRowProps) => {
  const variant = item.variant;
  const product = variant?.product;
  const productName = product?.name ?? "Unavailable Product";
  const media = product?.medias as Media[] | undefined;
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex gap-4">
          <div className="relative h-20 w-20 shrink-0 overflow-hidden rounded-lg bg-linear-to-br from-gray-100 to-gray-200">
            {imgUrl ? (
              <img src={imgUrl} alt={productName} className="h-full w-full object-cover transition-transform duration-300 group-hover:scale-110" />
            ) : (
          ...
          <div className="flex flex-col justify-center">
            <h3 className="font-semibold text-gray-800 line-clamp-1">{productName}</h3>
        ...
        <div className="flex items-center justify-between sm:gap-6">
          ...
          <div className="flex items-center gap-4">
            <span className="hidden font-bold text-gray-900 sm:inline">₦{itemTotal.toLocaleString()}</span>
            <button
              onClick={onRemove}
              disabled={isUpdating}
...
const ShippingCalculator = ({ currentDelivery, onShippingUpdate, showToast }: ShippingCalculatorProps) => {
  const [calculateShipping, { isLoading: isCalculating }] = useCalculateCartShippingMutation();
  const [updateDelivery] = useUpdateCartDeliveryMutation();
  const { data: states = [], isLoading: statesLoading } = useGetStatesQuery();
  const [selectedStateId, setSelectedStateId] = useState(currentDelivery?.deliveryStateId || "");
  const [selectedLgaId, setSelectedLgaId] = useState(currentDelivery?.deliveryLgaId || "");
  const { data: lgas = [], isLoading: lgasLoading, isFetching: lgasFetching } = useGetLgasByStateQuery(selectedStateId, {
  ...
  const handleCalculateShipping = async () => {
    if (!selectedStateId || !selectedLgaId) {
      showToast("Please select both state and local government area", "error");
      return;
    }
    try {
      const result = (await calculateShipping({ deliveryStateId: selectedStateId, deliveryLgaId: selectedLgaId }).unwrap()) as CalculatedShipping;
      // Update the cart with the chosen state/LGA (no shippingZoneId)
      await updateDelivery({ deliveryStateId: selectedStateId, deliveryLgaId: selectedLgaId }).unwrap();
      onShippingUpdate(result.deliveryFee || 0);
      showToast(`Shipping updated: ₦${(result.deliveryFee || 0).toLocaleString()}`, "success");
    } catch (error: any) {
      console.error("Shipping calculation error:", error);
      const errorMessage = error?.data?.message || "Failed to calculate shipping. Please try again.";
  ...
  return (
    <div className="mt-6 rounded-xl border border-gray-200 bg-linear-to-br from-gray-50 to-white p-4">
      <div className="flex items-center gap-2 mb-3">
        ...
        <h4 className="font-semibold text-gray-800">Estimate Shipping</h4>
      </div>
      <div className="space-y-3">
        ...
        <select
          ...
          disabled={!selectedStateId || lgasLoading || lgasFetching}
          className="w-full rounded-lg border border-gray-200 px-3 py-2 text-sm transition-all focus:border-green-500 focus:outline-none focus:ring-2 focus:ring-green-500/20 disabled:bg-gray-100 disabled:cursor-not-allowed"
        >
        ...
        <button
          ...
          disabled={isCalculating || !selectedStateId || !selectedLgaId || statesLoading || lgasLoading}
        ...
        >
          {isCalculating ? <Loader2 className="mx-auto h-4 w-4 animate-spin" /> : "Update Shipping"}
        </button>
      </div>
    </div>
  );
  ...
  return (
    <div className="sticky top-6 rounded-2xl border border-gray-200 bg-white p-6 shadow-lg transition-all hover:shadow-xl">
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-xl font-bold bg-linear-to-r from-gray-800 to-gray-600 bg-clip-text text-transparent">Order Summary</h3>
      ...
      <div className="space-y-3 border-b border-gray-100 pb-4">
        ...
        <div className="flex justify-between text-sm">
          <span className="flex items-center gap-1 text-gray-600"><Truck size={14} /> Shipping</span>
          <span className="font-medium text-gray-900">₦{shipping.toLocaleString()}</span>
        </div>
        {savings > 0 && (
          <div className="flex justify-between text-sm">
            <span className="flex items-center gap-1 text-green-600"><Percent size={14} /> Savings</span>
      ...
      <div className="mt-4 flex justify-between text-lg font-bold text-gray-900">
        <span>Total</span>
        <span className="text-2xl text-green-600">₦{grandTotal.toLocaleString()}</span>
      </div>
      <div className="mt-6 space-y-3">
        ...
        >
          <CreditCard size={18} className="transition-transform group-hover:-translate-y-0.5" /> Proceed to Checkout
        ...
        >
          {isClearing ? <Loader2 className="h-4 w-4 animate-spin" /> : <Trash2 size={16} />}
          Clear Cart
        </button>
      </div>
      <ShippingCalculator currentDelivery={deliveryInfo} onShippingUpdate={onShippingUpdate} showToast={showToast} />
...
const useCartActions = (showToast: (message: string, type: ToastType) => void) => {
  ...
  const handleRemove = async (item: CartItem) => {
    if (!item.variant) {
      showToast("Cannot remove item: product variant missing", "error");
      return;
    }
    try {
      setUpdatingItemId(item.id);
      await removeItem({ variantId: item.variant.id }).unwrap();
  ...
  return { updatingItemId, isClearing, handleIncrease, handleDecrease, handleRemove, handleClearCart };
};
...
const Cart = () => {
  const navigate = useNavigate();
  const { data: user, isLoading: isAuthLoading, isError: isAuthError } = useMeQuery();
  const { data, isLoading: isCartLoading, isError: isCartError, error: cartError } = useGetCartQuery(undefined, { skip: !user });
  const { toasts, showToast, removeToast } = useToast();
  const { updatingItemId, isClearing, handleIncrease, handleDecrease, handleRemove, handleClearCart } = useCartActions(showToast);
    ...
    return (
      <div className="flex min-h-[70vh] items-center justify-center p-4">
        <div className="flex max-w-md flex-col items-center gap-4 rounded-2xl border border-red-200 bg-linear-to-br from-red-50 to-orange-50 p-8 text-center shadow-lg">
          <div className="rounded-full bg-red-100 p-3"><AlertCircle className="h-10 w-10 text-red-500" /></div>
          <div className="space-y-2">
      ...
      <div className="min-h-screen bg-linear-to-br from-gray-50 via-white to-gray-50 py-8">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="mb-8 flex flex-wrap items-center justify-between gap-4 animate-in slide-in-from-top-5 fade-in duration-500">
            <div>
              <h1 className="text-4xl font-bold bg-linear-to-r from-gray-900 to-gray-600 bg-clip-text text-transparent">Shopping Cart</h1>
              <p className="mt-1 text-sm text-gray-500">Review and adjust your items before checkout</p>
            </div>
            <button
              onClick={() => navigate(-1)}
            ...
            >
              <ChevronLeft size={20} className="transition-transform group-hover:-translate-x-1" /> Continue Shopping
            </button>
          </div>
          <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
            <div className="space-y-4 lg:col-span-2">
              {items.map((item, index) => (
                <div key={item.id} className="animate-in slide-in-from-bottom-3 fade-in duration-300" style={{ animationDelay: `${index * 50}ms` }}>
                  <CartItemRow
                    item={item}
          ...
          <div className="mt-12 flex flex-wrap justify-center gap-6 border-t border-gray-200 pt-8">
            <div className="flex items-center gap-2 text-sm text-gray-500"><Shield size={16} className="text-green-600" /><span>Secure Payment</span></div>
            <div className="flex items-center gap-2 text-sm text-gray-500"><Clock size={16} className="text-green-600" /><span>14-Day Return Policy</span></div>
          </div>
        </div>
      </div>
    </>
